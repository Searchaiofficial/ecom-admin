...
const OfferForm = () => {
    ...
    const handleCreateOffer = async () => {
        try {
            ...
            const responce = await axios.post(`${BASE_URL}/api/createOffer`, { type: offerName, percentageOff: percentageOff })
    ...
    const handleAddProductToOffer = async () => {
        try {
            ...
            const responce = await axios.post(`${BASE_URL}/api/addProductToOffer`, { type: selectedOfferType, productId: selectedProduct })
            window.alert(responce.data.message)
    ...
    const handleDeleteProduct = async () => {
        try {
            const responce = await axios.patch(`${BASE_URL}/api/removeProductFromOffer`, { type: Deletedtype, productId: DeletedProduct })
            window.alert(responce.data.message)
    ...
    const handleDeleteOfferType = async () => {
        try {
            ...
            const responce = await axios.delete(`${BASE_URL}/api/deleteOffer/${encodedOfferType}`)
            window.alert(responce.data.message)
    ...
    const handleAddCategoryToOffer = async () => {
        try {
            ...
            const responce = await axios.post(`${BASE_URL}/api/addCategoryToOffer/`, { type: selectedOfferType, categoryType: selectedCategory })
            window.alert(responce.data.message)
    ...
    const handleRemoveCategory = async () => {
        try {
            const responce = await axios.patch(`${BASE_URL}/api/removeCategoryFromOffer/`, { type: deletedOfferForCategory, categoryType: deletedCategory })
            window.alert(responce.data.message)
        } catch (error) {
        ...
        <div>
            <div className="space-y-12 bg-white p-6 md:p-12 flex flex-col border-b">
                ...
                <div className="border-gray-500 border-b pb-12 w-full flex items-center justify-center gap-10">
                    ...
                    <div className="mt-5">
                        <button onClick={handleCreateOffer} className="bg-blue-500 text-white p-2.5 font-semibold rounded-lg uppercase">Create</button>
                    </div>
                </div>
 ...
 */}
                ...
                <div className="flex flex-col items-center gap-10 mb-12">
                    <h1 className="text-2xl font-bold leading-7 text-gray-900 text-center">Delete offer</h1>
                    <div className="flex items-center gap-36">
                        <div>
                            <lable className="text-sm font-medium  text-gray-900">Choose offer</lable>
                            <select
                        ...
                        <div>
                            <button onClick={handleDeleteOfferType} className="p-3 bg-blue-500 rounded-lg text-white  font-semibold">Delete Offer</button>
                        </div>
                    </div>
                </div>
