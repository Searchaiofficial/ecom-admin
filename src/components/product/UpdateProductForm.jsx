...
const UpdateProductForm = () => {
  ...
  const fetchProduct = async () => {
    try {
      const responce = await axios.get(
        `${BASE_URL}/api/fetchProductById/${id}`
      );
  ...
  const updateDemandType = async () => {
    try {
      const responce = await axios.post(
        `${BASE_URL}/api/addProductToDemandType`,
        { type: demandName, productId: id }
      );
  ...
  const updateSpecialPrice = async () => {
    try {
      const responce = await axios.patch(`${BASE_URL}/api/updateSpecialPrice`, {
        productId: id,
        specialprice,
      });
      window.alert(responce.data.message);
  ...
  const removeSpecialPrice = async () => {
    try {
      const responce = await axios.patch(
        `${BASE_URL}/api/removeSpecialPrice/${id}`
      );
      window.alert(responce.data.message);
    } catch (error) {
      console.log("ADD PRODUCT TO DEMAND ERROR :", error);
  ...
  const RemoveProductfromDemandType = async () => {
    try {
      const responce = await axios.patch(
        `${BASE_URL}/api/removeProductFromDemandType`,
        { type: demandName, productId: id }
      );
      window.alert(responce.data.message);
      setDemandName(null);
    ...
    <div>
      <div className="flex flex-col p-4 gap-10">
        ...
        <div className="flex items-center gap-12">
          ...
          <button
            onClick={RemoveProductfromDemandType}
            className="bg-red-500 text-white p-2 rounded-lg"
          >
            Remove Product From Demand type
          </button>
        </div>
        ...
        <div className="flex items-center gap-20">
          ...
          <button
            onClick={updateOfferType}
            className="bg-blue-500 text-white p-2 rounded-lg -ml-8"
          >
            Update Offer
          </button>
          <button
            onClick={RemoveProductfromOfferType}
            className="bg-red-500 text-white p-2 rounded-lg -ml-8 min-w-[275px]"
          >
        ...
        <div className="flex flex-col p-6 border gap-10  w-[50%]">
          <span>Price : {product?.perUnitPrice}</span>
          {product?.specialprice?.price && (
            <div className="flex flex-col gap-4">
              <span>Special Price : {product?.specialprice?.price}</span>

              <button
                onClick={removeSpecialPrice}
                className="bg-red-600 text-white p-2 w-fit rounded-md"
              >
          ...
          <div className="flex items-center gap-14">
            ...
            <input
              ...
              id="specialPrice"
              className="bg-gray-100 py-1 px-2 rounded-md min-w-[215px]"
            />
          </div>
          <div className="flex items-center gap-14">
            <label
          ...
          <div className="flex flex-col gap-2">
            <button
              ...
              className="bg-blue-500 p-3 text-white rounded-lg uppercase font-semibold"
            >
              Update
            </button>
            <button
              onClick={() => window.location.reload()}
              className="bg-gray-100 p-3 text-black border-2 rounded-lg uppercase font-semibold"
            >
              Cancel
            </button>
          </div>
